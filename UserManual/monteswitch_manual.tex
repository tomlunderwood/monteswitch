...
We now provide a worked example to elucidate how \emph{monteswitch} can be used to solve a `real' problem.
Specifically, we use \emph{monteswitch} to calculate the Gibbs free energy difference $\Delta G$ between the bcc and hcp
phases of Zr, modeled using the EAM potential (see Section \ref{sec:EAM}) of Ref. \cite{Mendelev_2007}, at 1234K and zero pressure. 
In our calculation we use artificial dynamics (see Section \ref{sec:transition_matrix}) 
to quickly determine a reasonable weight function, and \texttt{monteswitch\_mpi} to
parallelise the calculations. Below we go through the entire procedure to obtain $\Delta G$. 
Of course, the procedure described here will not be applicable to all situations. 
...
\item \texttt{weight\_function\_generation} is the simulation to generate the weight function for use in the forthcoming production simulation to
calculate $\Delta G$;
\item \texttt{weight\_function\_verification} is a short multicanonical simulation using the weight function obtained from the 
\texttt{weight\_function\_generation} simulation to ensure that it looks reasonable;
\item \texttt{production\_simulation} is the simulation used to calculate $\Delta G$, as well as thermodynamic quantities for each phase.
\end{itemize} 
...
subdirectory. 
Recall that the input files for `new' \texttt{monteswitch} and \texttt{monteswitch\_mpi} simulations (i.e., simulations invoked using the \texttt{-new} argument)
are \texttt{params\_in}, \texttt{lattices\_in}, and any input files for initialising the interatomic potential -- in this case \texttt{interactions\_in}
(see Section \ref{sec:EAM}).
Recall also that the output files for a `new' simulation are \texttt{F.dat}, \texttt{rho.dat} and \texttt{rphi.dat} (see Section \ref{sec:EAM} for a description 
of these files); \texttt{state} and \texttt{data} for serial simulations (using the \texttt{monteswitch} program); and additionally \texttt{data\_0}, 
\texttt{state\_0}, \texttt{data\_1}, \texttt{state\_1}, \texttt{data\_2}, \texttt{state\_2}, etc. for MPI simulations (using the \texttt{monteswitch\_mpi} program).
The simulations \texttt{bcc\_preliminary}, \texttt{hcp\_preliminary} and \texttt{weight\_function\_generation} are `new' simulations. Note that these
simulations all use identical \texttt{interactions\_in} and \texttt{lattices\_in}, copies of which can be found in the main
directory for the example. Their input files differ only in that they have different \texttt{params\_in} files. 
With this in mind we have annotated the \texttt{params\_in} files to highlight the salient variable choices for each simulation.

By contrast the simulations \texttt{weight\_function\_verification} and \texttt{production\_simulation} are `resumed' simulations (i.e., simulations invoked using 
either the \texttt{-resume} or \texttt{-reset} arguments). Recall that such simulations use an existing \texttt{state} file as input for the simulation, 
which is overwritten periodically throughout and at the completion of the simulation.
In subdirectories corresponding to `resumed' simulations we include both the \texttt{state} file used as input to the simulation, as well as the \texttt{state}
file output at the completion of the simulation. To distinguish between these files we have named the input \texttt{state} files \texttt{state\_start}. To restate, 
in directories corresponding to `resumed' simulations, \texttt{state\_start} is the input file for the simulation, while \texttt{state} is the output file.

Recall also that \texttt{monteswitch} and \texttt{monteswitch\_mpi} can output information to stdout. For simulations which output information to stdout,
the file \texttt{stdout} contains a copy of that output.
Finally, each subdirectory contains files which are the result of post-processing performed on the output files for the corresponding simulation. 
These files, and how they were created, are described in due course.
...
a \texttt{lattices\_in} file suitable for our purposes, as opposed to creating it from scratch. We know the density of both phases is about
0.043\AA$^{-3}$ at the temperature under consideration for Zr. Hence we use this as the first argument to \texttt{lattices\_in\_bcc\_hcp}.
We generated  \texttt{lattices\_in} using the command
\begin{verbatim}
$ ../../lattices_in_bcc_hcp 0.042710367 4 6 4 > lattices_in
\end{verbatim}
Recall that the second, third and fourth arguments to \texttt{lattices\_in\_bcc\_hcp} correspond to how many unit cells to tile in the
x-, y- and z-directions to create the supercell for each phase. As can be seen from inspection of the resulting \texttt{lattices\_in} 
file, a copy of which can be found in the main directory for the example, the above command yields a supercell of reasonable size (384 atoms), 
and supercell dimensions which are greater than twice the cut-off of the EAM potential (which is 7.6\AA, the last value on the second
...
Now we turn to simulations. Before performing LSMC simulations, it is necessary to first perform
preliminary conventional Monte Carlo simulations for each phase under consideration, in this case bcc and hcp. The directories \texttt{bcc\_preliminary}
...
\item \textbf{part\_step} and \textbf{vol\_step} are the maximum particle and volume move sizes. One of the aims of the preliminary simulations is
  ...
  or volume moves will be very low 
  ...
  to the total number of moves. If either is the case we simply stopped the simulation, and restarted with a better guess for
...
\item \textbf{output\_file\_period} is set to 1, which means information is output to the \texttt{data} file every sweep. Normally outputing information
  so frequently is excessive, but we need such high-resolution information regarding the time evolution of the simulation to 
...
\item \textbf{output\_stdout\_period} is set to 10: information is output to stdout every 10 sweeps. The information we are interested in while the simulation
  ...
  \textbf{part\_step} and \textbf{vol\_step} (acceptance rate = number of accepted moves / total moves)
...
Each of the preliminary simulations is run using the \texttt{-new} argument. Given that the simulation is short, it is unnecessary to use MPI: the 
\texttt{monteswitch} executable should be used. The following command invokes the executable (from within the \texttt{Examples/EAM\_Zr\_bcc\_hcp/bcc\_preliminary} or 
...
\end{verbatim}
Plotting this file immediately gives an idea of the range of order parameters exhibited by equilibrium states for each phase. We see that for
bcc the exhibited range is about -72 to -22, while for hcp the range is about 15 to 42. Hence we estimate that the order-parameter range which encompasses
equilibrium states belonging to \emph{both} phases is -82 to 48; we will use this order-parameter range in the forthcoming multicanonical simulations.
Note that we have deliberately `overestimated' the order-parameter range (specifically, by $\approx 15$\%) to take into account the fact that our 
preliminary simulations are relatively short, and hence only provide a semi-quantitative estimate of the order-parameter range for each phase: we overestimate
to `be on the safe side'.

Recall that in \emph{monteswitch} the system is constrained to be within the chosen order parameter range. It is therefore possible that imposing this artificial
...
the equilibration time, and the range of order parameters which encompasses the equilibrium states of both phases. With this information we can
now proceed to LSMC simulations. The first such simulation is to generate the weight function. The directory
\texttt{weight\_function\_generation} corresponds to this simulation. We use the artificial dynamics. Furthermore we use \texttt{monteswitch\_mpi}
...
to invoke the simulation (from within the \texttt{Examples/weight\_function\_generation} within the \emph{monteswitch} package itself) on our local machine:
\begin{verbatim}
$ mpiexec -n 4 ../../../monteswitch_mpi -new
\end{verbatim}
Note that this command may be different for the user's platform.
...
\item \textbf{M\_grid\_size}, \textbf{M\_grid\_min} and \textbf{M\_grid\_max} define the macrostates. We set \textbf{M\_grid\_min} and \textbf{M\_grid\_max}
  to -82 and 48, which corresponds to the
...
will not be exactly the case after our weight function simulation. Nevertheless we can obtain an \emph{approximate} probability distribution from the weight
...
\end{verbatim}
(Note that the appearance of \texttt{wf.dat} twice in the above command is deliberate).
...
With the macrostate probabilty distribution one can obtain an estimate of the free energy difference between the phases. Recall that the
free energy difference $\Delta G$ is related to the time spent in each phase via Eqn. \eqref{DeltaF_stat_mech}, where
where recall that $p_2/p_1$ is equal to $t_2/t_1$, and $t_1$ and $t_2$ are the total \emph{canonical} time the system spends in phases 1 
and 2. The time spent in each phase can be deduced by
integrating the aforementioned peaks in the macrostate probability distribution in \texttt{prob.dat}. 
The peak for negative order parameters corresponds
...
a range of order parameters of -72 to -22, while phase 2 (hcp) corresponded to a range of 15 to 42. 
...
applies Eqn. \eqref{DeltaF_stat_mech} to obtain the free energy difference from the weight function:
\begin{verbatim}
...
\end{verbatim}
Using this command gives $\Delta G$ to be 0.00113081eV per particle, which corresponds to the hcp phase being very slightly more favoured than
the bcc phase. We will compare this value with the `true' value we obtain for $\Delta G$ later.

We emphasise that the method just described provides only a crude estimate of $\Delta G$. One should not rely upon this approach 
for accurate results. However this method could be used, e.g., to quickly determine estimates for $\Delta G$ over a wide range of 
temperatures with the aim of deducing the approximate position of the coexistence temperature (where $\Delta G=0$). 
Note that, unlike our forthcoming production simulation, this method does not provide an uncertainty for $\Delta G$. 
...
The the control variables required to implement each of the aforementioned methods within \emph{monteswitch} are shown in Table 
...
method, and hence its value is unimportant}
\end{table}
\end{landscape}
...
in the canonical ensemble. As we will see in a moment, the equilibration times in the (two-phase, multicanonical) LSMC simulation are far longer. 
We need to know this equilibration time because it determines how large the blocks should be for block averaging to calculate thermodynamic quantities -- 
in particular the free energy difference. To determine this we perform a short multicanonical simulation using the weight function. This simulation also acts to
...
analyse the data -- once the production simulation is complete -- for a variety of block sizes, though software to do this is not included in
\emph{monteswitch}.
...
provides an easy way of extracting this information; the following command creates a file \texttt{counts_vs_M.dat} which is a plot of the order parameter for each macrostate (the lower bound for the macrostate) vs. the number of times the  macrostate was visited during the entire simulation:
\begin{verbatim}
...
mean-squared displacements of each particle) and their uncertainties using block averaging.
...
\subsection{Results}
At the completion of the simulation the \texttt{state} file contains the equilibrium quantities, evaluated using block averaging. It is these quantities we
...
\textbf{sigma\_equil\_DeltaF} in \texttt{state} contain the free energy difference and its uncertainty evaluated using block averaging.
...
'{print $1,$2/384}'
\end{verbatim}
The final value for $\Delta G$ is therefore 0.00104(2)eV per particle, (c.f. 0.00113eV per particle obtained from the weight function earlier)
...
difference, which is the case here, suggests that hcp (phase 2 here) is stable over bcc at the considered temperature and pressure. 
However, note that the free energy difference between the phases is $\sim$1meV, which is very small. Hence this temperature and pressure can be 
considered to be a coexistence point to within an uncertainty of $\sim 1$meV/atom. This is in agreement with Ref. \cite{Mendelev_2007}, in which
it was found that this temperature and pressure corresponded to a coexistence point.
...
\begin{tabular}{l | l | l | l | l }
                                          &  \emph{monteswitch}      & Ref. \cite{Mendelev_2007} \\
                                                        \hline
$\Delta G_{hcp\to bcc}$                   & 0.00104(2)               & 0                         \\
$\Delta H_{hcp\to bcc}$                   & 0.0394(2)                & 0.039                     \\
$\Delta V_{hcp to bcc}/V_{bcc}$ (\%)      & -0.812(7)                & -0.8                      \\
\end{tabular}
\caption{All energies quoted in this table are in eV, and all volumes are in \AA$^3$. All quantities are intensive, i.e., `per atom'.}
\end{table}
...
simulation to determine the appropriate block size. However, to check this more explicitly we plot the order parameter vs. simulation time for each
MPI thread:
\begin{verbatim}
...
\end{verbatim}
Plotting the files \texttt{M\_vs\_t\_0.dat}, \texttt{M\_vs\_t\_1.dat}, \texttt{M\_vs\_t\_2.dat} and \texttt{M\_vs\_t\_3.dat} it can be seen that the 
system traverses the entire range of order-parameter space (-62--32) often within the block size (175000 sweeps), as required.
