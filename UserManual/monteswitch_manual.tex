...
\end{equation}
where $u_i$ is the displacement of particle $i$ its notional lattice site, $N$ denotes the number of particles in the system, and $\alpha$
...
\end{equation}
(where $\beta=1/(k_BT)$, with $T$ the temperature of the system), the mean-squared displacement of each particle is \cite{book:Frenkel}
...
of $\alpha$. We denote the $\alpha$ pertaining to phase 1 as $\alpha_1$, and similarly for phase 2. Note that the free energy difference
...
\end{equation}
where we have used Eqn. \eqref{EC_F}. We use Eqns. \eqref{EC_F}, \eqref{EC_u2} and \eqref{EC_DeltaF} below.

The relevant \texttt{interactions.f95} file for these tests is \texttt{interactions\_EC.f95}. Before running these 
tests one should copy \texttt{Interactions/interactions\_EC.f95} to \texttt{interactions.f95}, and then complile 
\emph{monteswitch}. The Hamiltonian for each phase realised in \emph{monteswitch} in conjunction with \texttt{interactions\_EC.f95} is 
the Einstein crystal as described above, where note that the orign constitutes the lattice site \emph{for all particles}.
$\alpha_1$ and $\alpha_2$ are specified for new simulations via the input file \texttt{interactions\_in}. 
The format of this file is as follows. On the first line there are two tokens. The first token is a \texttt{CHARACTER(LEN=20)} 
variable (I recommend: \texttt{alpha\_1=}). The the second token is the value of $\alpha_1$ (stored internally as 
...
\begin{enumerate}
\item
$\Delta F$ should be $0.015\ln(1/2)=-0.0103972$. Check that this is the case for the simulation: the $\Delta F$ is stored in 
...
1, 2 and 3 described in \texttt{test\_EC\_1} applies to the variables listed in the \texttt{state} file here, instead of the
output to stdout. This test essentially verifies that \texttt{monteswitch\_mpi} works correctly, especially that it correctly pools 
...
This test calculates the thermodynamic quantities highlighted in \texttt{test\_EC\_1}, but uses multicanonical sampling instead
...
This test is similar to \texttt{test\_EC\_3}, but instead generates the weight function using the shooting method.
Here a weight function generation simulation is performed which consists of 1,000,000 sweeps. Then the \texttt{state} 
file is altered in preparation for the production simulation. Then the production simulation is performed, which 
consists of 10,000,000 sweeps. 
Checks 1, 2 and 3 described in \texttt{test\_EC\_1} should be performed on the \texttt{state} file.
...
This test is similar to \texttt{test\_EC\_3}, but instead generates the weight function using the shooting method 
in conjunction with `\texttt{"pong\_down"} artificial dynamics' and canonical sampling. The  weight function
generation simulation consists of 1,000,000 sweeps. Two further files are created 
during this test: \texttt{M\_vs\_t.dat} gives the value of the order parameter vs. simulation time (data points 
every 100 sweeps), and \texttt{macro\_vs\_t.dat} similarly gives the macrostate number vs. simulation time. 
Both plots should resemble a sawtooth.
...
artificial dynamics' are used. The files \texttt{M\_vs\_t\_0.dat} and \texttt{macro\_vs\_t\_0.dat} pertain to thread '0', and
...
This test is similar to \texttt{test\_EC\_5}, except that `\texttt{"random"} artificial dynamics' are used. The files 
...
sweeps) instead of 1. This tests continuity of simulations using artificial dynamics over checkpoints.
Checks 1, 2 and 3 described in \texttt{test\_EC\_1} should be performed on the \texttt{state} file.
...
in the output to stdout. Recall that 'H' in monteswitch refers to the energy in an NVT simulation, and 
...
1, 2 and 3 described in \texttt{test\_EC\_8} apply to the variables listed in the \texttt{state} file as opposed to the output to
stdout. This test essentially verifies that \texttt{monteswitch\_mpi} works correctly for multiple particles, especially that it 
...
The Einstein crystal cannot be used to validate \emph{monteswitch} for the NPT ensemble. Accordingly we consider here a modification of the Einstein
crystal model suitable for this purpose. We refer to this model as the \emph{NPT Einstein crystal}. In the NPT Einstein crystal the energy of the 
system is given by
\begin{equation}
...
The following analytical results can be derived for this model in the NPT ensemble. Firstly, the partition function is given by
\begin{equation}
Z=\bigl[2\pi\Gamma(1.5)\bigr]^N(\beta\alpha)^{-1.5N}(\beta P)^{-(1.5N\gamma+1)}\Gamma(1.5N\gamma+1),
\end{equation}
where $\Gamma(x)$ is the gamma function, $P$ is the pressure of the system, $\beta=1/(k_BT)$, and $T$ is the temperature of the system. 
From this the Gibbs free energy difference $\Delta G$ between two NPT Einstein crystals can be derived:
...
\end{equation}
and we have indicated the quantities pertaining to phase 1 and phase 2 by subscripts. Furthermore, it can be shown that the mean-squared displacement 
...
\end{equation}
and that the mean volume is given by
...
The relevant \texttt{interactions.f95} file for these tests is \texttt{interactions\_EC\_NPT.f95}. Before running these 
tests one should copy \texttt{Interactions/interactions\_EC\_NPT.f95} to \texttt{interactions.f95}, and then complile 
\emph{monteswitch}. The Hamiltonian for each phase realised in \emph{monteswitch} in conjunction with 
...
The format of this file is as follows. On the first line there are two tokens. The first token is a \texttt{CHARACTER(LEN=20)} 
variable (I recommend: \texttt{alpha\_1=}). The the second token is the value of $\alpha_1$ (stored internally as 
\textbf{alpha\_1}), which is of type \texttt{REAL}. The second line is similar, but for $\alpha_2$. The third is similar for $\gamma_1$, 
...
instead of one simulation of 10,000,000 sweeps, using both the \texttt{-resume} and \texttt{-new} arguments to the executable 
\texttt{monteswitch} instead of simply the \texttt{-new} argument. This test essentially verifies that checkpointing works
correctly. Checks 1--3 described in \texttt{test\_EC\_NPT\_1} should be performed on the \texttt{state} file.
...
4,000,000 sweeps. This test essentially verifies that checkpointing works correctly for \texttt{monteswitch\_mpi}. 
Checks 1--3 described in \texttt{test\_EC\_NPT\_2} should be performed on the \texttt{state} file.


\subsection{\texttt{test\_EC\_NPT\_3}}
This test calculates the thermodynamic quantities as described in \texttt{test\_EC\_NPT\_1}, but uses multicanonical sampling,
...
Checks to perform beyond  1--3 described in \texttt{test\_EC\_NPT\_1}:
\begin{enumerate}
\setcounter{enumi}{3}
\item
The macrostate histogram in \texttt{M\_hist.dat} should be fairly flat, which reflects the fact that all macrostates are
visted with equal probability in multicanonical sampling using an ideal weight function.
\end{enumerate}


\subsection{\texttt{test\_EC\_NPT\_4}}
This test is similar to \texttt{test\_EC\_NPT\_2}, but uses $N=5$ instead of $N=1$, and a lattice switch which changes the system volume.
