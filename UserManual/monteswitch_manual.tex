...
[REFERENCE MY CONFERENCE PAPER?]

%[FREE ENERGY DIFFERENCES ARE IMPORTANT; THERE ARE A PLETHORA OF METHODS AVAILABLE, EACH SUITABLE FOR DIFFERENT TASKS; LSMC IS GOOD FOR...]
Calculating the free energy difference between two given phases is one of the most fundamental problems in materials science.
A plethora of different methods have been developed to this end, each designed with a particlular problem in mind.
%
\emph{Lattice-switch Monte Carlo} (LSMC) \cite{Bruce_1997,Bruce_2000} 
\footnote{The reader should be aware that LSMC has also been refered to as lattice-\emph{switching} Monte Carlo. Furthermore the
term \emph{phase switch Monte Carlo}, which is discussed in a moment, is usually used in the literature to refer to the derivative of
LSMC suitable for calculating the free energy difference between a solid and a fluid phase.}
...
between the fcc and hcp phases \cite{Bruce_1997,Bruce_2000}, as well as the free energy cost of stacking faults
...
species of particle \cite{Jackson_2007,Yang_2008}, and molecular systems \cite{Marechal_2008,Raiteri_2010,Quigley_2014,Bridgwater_2014}.
...
more efficient than other existing methods [REFS]. 
However it should be noted that this remains somewhat of an open question [REFS]; nevertheless one can say that LSMC is \emph{at least} 
as efficient as existing methods. 
...
While steps have been recently been taken to implement LSMC in existing general-purpose Monte Carlo codes, we believe that \emph{monteswitch} will fulfill
an important `gap in the market' for the foreseeable future for two reasons. Firstly, \emph{monteswitch} is a bespoke LSMC code, and as such
one would expect it to require less computational resources than general-purpose codes for similar LSMC calculations. Secondly, the package
was designed from the outset to be highly-customisable with regards to the interatomic potential. This is achieved by locating all of the source code
pertaining to the interatomic potential within a signle Fortran module housed within a single file \texttt{interactions.f95}. While various
versions of this file are included with \emph{monteswitch}, each corresponding to a commonly-used class of interatomic potential, 
...
\emph{monteswitch}.
To this end templates and examples provided with \emph{monteswitch} for guidance. Of course, in writing their own version of \texttt{interactions.f95} users 
are free to interface with external Fortran modules, or even external programs. After compiling the package with the desired version of 
\texttt{interactions.f95}, the end result is a version of \emph{monteswitch} in which the desired interatomic potential is `hard-coded'.
...
system is allowed to re-equilibrate before statistics are gathered. Also, the current block is disregarded for the purposes of block averaging. \\
...
but instead uses the state of the system specified in \texttt{state}. By contrast, the command-line argument \texttt{-new} initialises the 
...
all replicas, the statistics from all are combined, and the results are stored in the file \texttt{state}. The state stored in state
...
the same state} when the \texttt{-resume} argument is used with \texttt{monteswitch\_mpi}. In future we hope to allow `true' checkpointing 
...
Each program takes four arguments. The first is the density, i.e., the number of atoms per unit volume, of the pair of states to
