...
\subsection{Results}
[DOWN TO HERE]
In each of the \texttt{bcc\_preliminary} and \texttt{hcp\_preliminary} directories one can post-process the file \texttt{data} to create a plots of 
the system's energy, volume and order parameter vs. simulation time, where by simulation `time' here we mean the number of completed Monte Carlo sweeps. The 
files \texttt{E\_vs\_t.dat}, \texttt{V\_vs\_t.dat} and \texttt{M\_vs\_t.dat} in each of the directories correspond to such plots, and were created using the 
following commands invoked from within the directories:
\begin{verbatim}
...
from the figure, the system equilibrates quickly: within 200 sweeps. It can also be seen from the bottom panel in the figure that for
bcc the exhibited order-parameter range at equilibrium is about -72 to -22, while for hcp it is about 15 to 42. Hence we estimate that the 
order-parameter range which encompasses equilibrium states belonging to \emph{both} phases is -82 to 48. We will use this order-parameter range 
in the forthcoming LSMC simulations. Note that we have deliberately `overestimated' the order-parameter range (specifically, by $\approx 15$\%) 
to take into account the fact that our preliminary simulations are relatively short, and hence only provide a semi-quantitative estimate of 
the order-parameter range for each phase: we overestimate to `be on the safe side'.

Recall that in \emph{monteswitch} the system is constrained to be within the chosen order parameter range. It is therefore possible that 
imposing this artificial constraint on the system could invalidate the results of the simulation. To avoid this, one should choose the 
maximum and minimum of the order-parameter range to be values which are `sufficiently unlikely to be realised at equilibrium so not to affect 
the results'. At this point it is not clear whether this is the case for our chosen range. Fortunately we can check this \emph{a postiori} 
using the results of later simulations (see Section \ref{section:double_check} below). On the other hand the maximum and minimum should 
not be too far from what is likely to be encountered at equilibrium. 
...
provides an easy way of extracting this information; the following command creates a file \texttt{counts\_vs\_M.dat} which is a plot of the order 
parameter for each macrostate (the lower bound for the macrostate) vs. the number of times the  macrostate was visited during the entire simulation:
\begin{verbatim}
