...
!   Language: Fortran 90
!
! Remarks and changes to the code made by Tom Underwood:
!   (1) I changed the indentation and spacing to make the code more readable.
!   (2) I adjusted the location of all '&' symbols - the code would not compile
!       because they were not at the end of a line to be continued, but at the 
!       beginning of the 'next' line. 
!   (3) I changed the declaration of the integer 'umask' from 'umask=-2147483648' to 
!       'umask=-2147483647-1'. The former gave an overflow error when I compiled the
!       code. After scouring the internet I found the problem. The range of 4 byte 
!       integers is -2147483648 to 2147483647, and hence the declaration 
!       'umask=-2147483648' should be valid. However, the evaluation of this 
!       expression by Fortran is apparently as follows: take 2147483648, and then make
!       it negative. Since 2147483648>2147483647, an overflow error is reported at
!       compile time. I got around this by making the aforementioned change to the
!       declaration of 'umask'.
!  WARNING: The procedures 'random_number' and 'random_seed' have the same names as
!       the fortran random number intrinsic procedures. This could cause problems if
!       one is not VERY careful about the arguments of these procedures. Consider
!       'random_number'. The version in this module supports (for the first argument)
!       only real arrays of rank 1 of the default kind. Hence if 'mersenne_twister'
!       is used, and 'random_number' is called with a real array of rank 1 of the 
!       default kind as its first argument, the Mersenne twister version is called,
!       not Fortran's intrinsic 'random_number' procedure. However, if
!       'mersenne_twister' is used and the first argument is a scalar real (of any 
!       kind), or a real array of rank 1 of a kind other than the default, then
!       Fortran's intrinsic 'random_number' procedure is called instead of the
!       Mersenne twister procedure - because there is no Mersenne twister
!       'random_number' procedure with these arguments to override the intrinsic
!       version. To avoid accidentally calling the intrinsic procedure when one
!       wants the Mersenne twister version, I recommend to not use this module
!       directly to generate random numbers, but to instead use a wrapper of some 
!       kind.
!    
!$$$
