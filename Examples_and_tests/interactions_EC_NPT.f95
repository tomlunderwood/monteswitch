  ...
  !! <html>
  !! <head>
  !!  <title> Einstein crystal NPT interactions Documentation </title>
  !! </head>
  !! <body>
  !! 
  !! <h1> Einstein crystal NPT <code>interactions</code> Documentation </h1>
  !!
  !! <h2> Author </h2>
  !! <p> Tom Underwood </p>
  !!
  ...
  !! </p>
  !! <p>
  !! This version of the file corresponds to a pair of Einstein crystals whose spring constants vary with
  !! the volume of the system: in lattice 1 the energy of each particle corresponds to a harmonic oscillator 
  !! about the lattice site in which the spring constant is <code>2*alpha_1*V^(-gamma_1)</code>, where <code>V</code>
  !! denotes the volume of the system; similarly the spring constant for lattice 2 is <code>2*alpha_2*V^(-gamma_2)</code>;
  !! the energy of a particle in lattice 1 is <code>alpha_1*V^(-gamma_1)*u^2</code>, where <code>u</code> is the displacement
  !! of the particles from its lattice site, and similarly for lattice 2.
  !! The free parameters are <code>alpha_1</code>, <code>gamma_1</code>, <code>alpha_2</code> and <code>gamma_2</code>.
  !! </p>
  !! <p>
  ...
  !! the second is the value of <code>alpha_1</code>. The second line is similar, but for <code>alpha_2</code>. The
  !! third is similar for <code>gamma_1</code>, and the fourth is similar for <code>gamma_2</code>.
  !! </p>
  ...
  !!  <td> Determines the 'scale' of the strength of the harmonic potential well centred on all lattice sites in phase 1
  !!  (see above). </td>
  !! </tr>
  !! <tr>
  !!  <td> <code>alpha_2<code> </td>
  !!  <td> <code>real(rk)</code> </td>
  !!  <td> Determines the 'scale' of the strength of the harmonic potential well centred on all lattice sites in phase 2
  !!  (see above). </td>
  !! </tr>
  !! <tr>
  !!  <td> <code>gamma_1<code> </td>
  !!  <td> <code>real(rk)</code> </td>
  !!  <td> Exponent of the power law volume dependence of the strength of the harmonic potential well centred on all lattice sites in phase 1
  !!  (see above). </td>
  !! </tr>
  !! <tr>
  !!  <td> <code>gamma_2<code> </td>
  !!  <td> <code>real(rk)</code> </td>
  !!  <td> Exponent of the power law volume dependence of the strength of the harmonic potential well centred on all lattice sites in phase 2
  !!  (see above). </td>
  !! </tr>
  !! </table>
  real(rk) :: alpha_1
...
contains
  ...
  subroutine initialise_interactions(filename)
    ...
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'gamma_1' from file '",filename,"'"
       stop 1
    end if
    read(10,*,iostat=error) string, gamma_2
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'gamma_2' from file '",filename,"'"
       stop 1
    end if
    close(10)
  ...
  subroutine export_interactions_state(unit)
    ...
    write(unit,*) "gamma_1= ",gamma_1
    write(unit,*) "gamma_2= ",gamma_2
  ...
  subroutine import_interactions_state(unit)
    ...
    read(10,*,iostat=error) string, gamma_1
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'gamma_1' from unit ",unit
       stop 1
    end if
    read(10,*,iostat=error) string, gamma_2
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'gamma_2' from unit ",unit
       stop 1
    end if
    ...
    do i=1,n_part
       ...
       case(1)
          ...
          calc_energy_scratch=calc_energy_scratch + &
               alpha_1*(Lx*Ly*Lz)**(-gamma_1) * min_image_distance(pos,site,Lx,Ly,Lz)**2
       case(2)
          pos=r(i,:)
          site=R_2(i,:)
          calc_energy_scratch=calc_energy_scratch + &
               alpha_2*(Lx*Ly*Lz)**(-gamma_2) * min_image_distance(pos,site,Lx,Ly,Lz)**2
       case default
  ...
  function calc_energy_part_move(lattice,Lx,Ly,Lz,r,r_new,i)
    ...
    case(1)
       ...
       calc_energy_part_move=alpha_1*(Lx*Ly*Lz)**(-gamma_1) * &
            ( min_image_distance(pos_new,site,Lx,Ly,Lz)**2 - min_image_distance(pos,site,Lx,Ly,Lz)**2 )
    case(2)
       pos=r(i,:)
       pos_new=r_new(i,:)
       site=R_2(i,:)
