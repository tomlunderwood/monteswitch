  ...
  ! interactions.f95 for Gaussian potential: -A*exp(-B*r^2)
...
contains
  ...
  subroutine initialise_interactions(filename)
    ...
    if(error/=0) then
        write(0,*) "interactions: Error. Problem reading 'B' from file '",filename,"'"
        stop 1
    end if
    ! <<<<<<<<<<<< END OF USER-DEFINED CODE >>>>>>>>>>>> 
  ...
  subroutine import_interactions_state(unit)
    ...
    read(unit,*,iostat=error) string, B
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'B' from unit ",unit
       stop 1
    end if
    ! <<<<<<<<<<<< END OF USER-DEFINED CODE >>>>>>>>>>>> 
