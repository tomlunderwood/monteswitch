  ...
  ! interactions.f95 file for Morse potential: E0*(  ( 1 - exp(-k*(r-r0)) )^2 - 1  )
...
contains
  ...
  subroutine initialise_interactions(filename)
    ...
    if(error/=0) then
        write(0,*) "interactions: Error. Problem reading 'k' from file '",filename,"'"
        stop 1
    end if
  ...
  subroutine import_interactions_state(unit)
    ...
    read(unit,*,iostat=error) string, k
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'k' from unit ",unit
       stop 1
    end if
  ...
  function pair_potential(r)
    ...
    !
    pair_potential= E0*(  ( 1.0_rk - exp(-k*(r-r0)) )**2 - 1.0_rk  )
    ! <<<<<<<<<<<< END OF USER-DEFINED CODE >>>>>>>>>>>> 
