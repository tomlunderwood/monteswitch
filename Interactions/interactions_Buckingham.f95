  ...
  ! interactions.f95 file for Buckingham potential: A*exp(-r/rho) - C/r^6
...
contains
  ...
  subroutine initialise_interactions(filename)
    ...
    if(error/=0) then
        write(0,*) "interactions: Error. Problem reading 'A' from file '",filename,"'"
        stop 1
    end if
    read(10,*,iostat=error) string, rho
    if(error/=0) then
        write(0,*) "interactions: Error. Problem reading 'rho' from file '",filename,"'"
        stop 1
    end if
    read(10,*,iostat=error) string, C
    if(error/=0) then
        write(0,*) "interactions: Error. Problem reading 'C' from file '",filename,"'"
        stop 1
    end if
    ! <<<<<<<<<<<< END OF USER-DEFINED CODE >>>>>>>>>>>> 
  ...
  subroutine import_interactions_state(unit)
    ...
    !  end if
    !
    read(unit,*,iostat=error) string, A
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'A' from unit ",unit
       stop 1
    end if
    ...
    read(unit,*,iostat=error) string, C
    if(error/=0) then
       write(0,*) "interactions: Error. Problem reading 'C' from unit ",unit
       stop 1
    end if
    ! <<<<<<<<<<<< END OF USER-DEFINED CODE >>>>>>>>>>>> 
